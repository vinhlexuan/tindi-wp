jQuery(document).ready(function($) {
    ...
    console.log('AI Post Generator Plugin - Admin JS loaded');

    // Thêm HTML cho phần hiển thị số dư và modal
    $('body.toplevel_page_post-generator-plugin #wpcontent, body.ai-post-generator_page_post-generator-plugin-settings #wpcontent, body.ai-post-generator_page_post-generator-plugin-logs #wpcontent').prepend(`
        <div id="ai-post-generator-balance" style="
            position: fixed;
            top: 32px;
            right: 0;
            background: #fff;
    ...
    const isBalanceVisible = localStorage.getItem('aiPostGeneratorBalanceVisible') !== 'false';
    if (!isBalanceVisible) {
        $('#balance-content').hide();
        $('#toggle-balance .dashicons').removeClass('dashicons-arrow-right').addClass('dashicons-arrow-left');
        $('#ai-post-generator-balance').css('transform', 'translateX(calc(100% - 40px))');
    }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('#plugin-name').text(data.plugin_name || 'AI Post Generator');
                    ...
                    $('#plugin-name').text('AI Post Generator');
    ...
    $('.ai-post-generator-admin-page').on('click', '.refresh-button', function(e) {
        e.preventDefault();
